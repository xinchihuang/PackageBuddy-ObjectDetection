...
def main(args=None):
	...
	while (True):
		...
		if max(r['scores'])>0.9:
			print("Door detected!")
		siamese_utils.display_results(ref_images[0], query_image, r['rois'], r['masks'], r['class_ids'],r['scores'])
	...
	cap.release()
	# for filename in os.listdir(ROOT_DIR + "testimage/test/move2/"):
	# 	print(filename)
	# 	query_image = io.imread(ROOT_DIR + "testimage/test/move2/" + filename)
	# 	print(query_image)
	# 	# Run detection
	# 	results = model.detect_category(category=category, targets=[ref_images], images=[query_image], verbose=1)
	# 	r = results[0]
	...
	# 		siamese_utils.display_results(ref_images[j], query_image, r['rois'], r['masks'], r['class_ids'],
	# 									  r['scores'])
	# 	break
